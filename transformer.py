...
class MultiHeadAttention(nn.Module):
    def __init__(self, n_head, d_model, d_key, d_value, dropout=0.1):
        ...
        self.W_Q = nn.Linear(d_model, n_head * d_key, bias=False)
        self.W_K = nn.Linear(d_model, n_head * d_key, bias=False)
        self.W_V = nn.Linear(d_model, n_head * d_value, bias=False)

        self.attention = ScaledDotProductAttention(temperature=d_key ** .5, dropout=dropout)
        self.ff = nn.Linear(n_head * d_value, d_model, bias=False)
...
class EncoderLayer(nn.Module):
    def __init__(self, d_model, n_head, d_feedforward, d_key, d_value):
        super(EncoderLayer, self).__init__()

        self.self_attention = MultiHeadAttention(d_model=d_model, n_head=n_head, d_key=d_key, d_value=d_value)
...
class Encoder(nn.Module):
    def __init__(self, d_input, d_model, n_head, n_layers, d_feedforward, d_key, d_value, dropout=0.1):
        ...
        self.layers = nn.ModuleList(
            [EncoderLayer(d_model, n_head, d_feedforward, d_key, d_value) for _ in range(n_layers)])
...
class Transformer(nn.Module):
    def __init__(self, d_input, d_target, max_length, d_model, n_head, n_layers, d_feedforward, d_key, d_value,
                 dropout=0.1):
